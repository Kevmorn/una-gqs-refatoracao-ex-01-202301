...
public class Validacao {
  public void calcularValidacaoFinal(Pessoa pessoa, Mensagem mensagem, boolean erroTentativas) {
    // Validando se eu devo imprimir a mensagem de erro
    if (erroTentativas == true) {
      ...
      if (pessoa.estaRetornandoViagem.equals("SIM")) {
        ...
        if (pessoa.porcentagemInfeccao >= 90) {
          pessoa.orientacaoFinal = "Paciente crítico! Por favor, aguarde em lockdown por 10 dias para ser acompanhado.";
          mensagem.imprimirRelatorioFinal(pessoa);
        } else {
          pessoa.orientacaoFinal = "Paciente com risco de estar infectado. Por favor, aguarde em lockdown por 02 dias para ser acompanhado.";
          mensagem.imprimirRelatorioFinal(pessoa);
        }
      }
    }
  ...
  public void validacaoDasProximasPerguntas(Mensagem mensagem, Pessoa pessoa, boolean erroTentativas, Scanner scanner) {
    // Validando se devo continuar perguntando ou ir diretamente para a mensagem final
